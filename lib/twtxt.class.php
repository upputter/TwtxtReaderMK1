...
Class Twtxt {
    ...
    public function parseTwtxtData(string $url, string $data) {
        if (mb_strlen($data) > 0) {            
            ...
            foreach ($lines AS $currentLine) {
                $lineCounter++;
                if (empty(trim($currentLine))) continue;
    ...
    public function getTwtxt($url = '') {
        ...
        if (str_contains($url, 'gemini://')) return false;      
        ...
        if (!is_null($this->entries)) {
            if (strtolower($this->config['sorting']) == 'desc') {
                @usort($this->entries, function($a, $b) { return ($a->dateTime <= $b->dateTime) ? true : false;});
            } else {
                @usort($this->entries, function($a, $b) { return ($a->dateTime >= $b->dateTime) ? true : false;});
            }           
        }
    ...
    public function filerEntriesByHash($hash) {
        ...
        // sort conversation
        @uasort($this->entries, function($a, $b) { return ($a->dateTime >= $b->dateTime) ? true : false;});
        
        // stack sorted entries together
        if ($this->showAsConversation) $this->buildConversation();
    }
    ...
    public function unique_entries(bool $limitEntries = true) {
        ...
        if ($limitEntries AND count($this->entries) > 0) {
            if ($this->limitMaxEntries) $this->entries = array_slice($this->entries, 0, $this->config['maxEntries']);
        }
    }
}